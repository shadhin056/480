...
if (mysqli_num_rows($result) > 0) {
   header("Location: Assignment07adminSuccess.php");
	exit();
	mysqli_close($conn);
    }
 else {
...
?>
	...
	<div class="container">
		...
		<header>
			<h4>Welcome To Sign In (Admin Account)</h4>
		</header>
		...
		<article>
			...
			<div style="margin-right:50px;margin-left:50px">
				<form method="post" target="_self" autocomplete="on" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
					...
					<input type="email" id="fname" name="email22" value="<?php if(isset($_POST['email22'])) echo $_POST['email22']; ?>" placeholder="shadhin@gmail.com" required>
				...
				<a href="Assignment07.php" target="_self"><button class="button"><span>Don't have account ?</span></button>
						</a>
			</div>
