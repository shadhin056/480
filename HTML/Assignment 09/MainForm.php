...
<script>
	function ValidateContactForm(){
		...
		var Tfull_name=document.getElementById('firstname').value;
		var Tfull_name2=document.getElementById('firstname');
		var jnick_name=ContactForm.lastname;
		var Tjnick_name1=document.getElementById('firstname').value;
		var Tjnick_name2=document.getElementById('firstname');
		...
		if( /[^a-zA-Z\-\/]/.test( Tfull_name ) ) {
		window.alert("Input is not alphanumeric");
        jfull_name.focus();
        return false;
    }
		
		if(Tfull_name2.checkValidity() == false){
			window.alert(Tfull_name2.validationMessage);
        jfull_name.focus();
        return false;
	}
		...
		if( /[^a-zA-Z0-9\-\/]/.test( Tjnick_name1 ) ) {
		window.alert("Input is not alphanumeric");
        jfull_name.focus();
        return false;
    }
		
		if(Tjnick_name2.checkValidity() == false){
			window.alert(Tfull_name2.validationMessage);
        jfull_name.focus();
        return false;
	}
		...
		<article>
			...
			<div style="margin-right:50px;margin-left:50px">
				...
				<?php/* echo htmlspecialchars($_SERVER["a.php"]);*/?>
					...
					<input type="text" id="Hfull_name" name="firstname" value="<?php if(isset($_POST['firstname'])) echo $_POST['firstname']; ?>" placeholder="Full Name (Character Min:2 Max:20)" min="2" max="20" autofocus >
